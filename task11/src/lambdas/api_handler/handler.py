...
def signin_user(body):
    ...
    return create_response(200, json.dumps({'accessToken': access_token}))
...
def create_table(body):
    ...
    return create_response(200, json.dumps({'id': body['id']}))
...
def fetch_tables():
    ...
    return create_response(200, json.dumps({'tables': tables}, cls=Encoder))
...
def fetch_table_by_id(table_id):
    ...
    return create_response(200, json.dumps(table_data, cls=Encoder))
...
def create_reservation(body):
    ...
    if not table_response['Items']:
        return create_response(400, 'Table not found')
    ...
    if reservations_response['Items']:
        return create_response(400, 'Conflicting reservation exists')

    body['id'] = str(uuid.uuid4())
    reservations_table.put_item(Item=body)

    return create_response(200, json.dumps({'reservationId': body['id']}))
...
def fetch_reservations():
    ...
    return create_response(200, json.dumps({'reservations': tables}, cls=Encoder))
    ...
    def handle_request(self, event, context):
        ...
        try:
            if resource == '/signup' and http_method == 'POST':
                ...
                return create_response(400, 'Unknown resource')
